...
if (!isLoggedIn()) {
    echo json_encode(['success' => false, 'message' => 'Please login to add items to cart.', 'redirect' => '/auth/login.php']);
    exit();
}

if (getUserRole() !== 'user') {
    echo json_encode(['success' => false, 'message' => 'Only users can add items to cart.']);
    exit();
}
